...
function conVersion(url, version) {
  if (!url) return url;
...
module.exports = async function handler(req, res) {
  ...
  const version = config?.updated_at ? new Date(config.updated_at).getTime() : null;
  ...
  res.status(200).json({
    ...
    foto_url: conVersion(storagePublicUrl('sitio', config?.foto_perfil_path), version),
    logo_url: conVersion(storagePublicUrl('sitio', config?.logo_path), version),
  });
};
